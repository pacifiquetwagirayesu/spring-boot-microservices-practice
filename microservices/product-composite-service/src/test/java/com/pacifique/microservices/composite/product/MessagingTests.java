...
import com.pacifique.microservices.api.composite.product.ProductAggregate;
import com.pacifique.microservices.api.composite.product.RecommendationSummary;
import com.pacifique.microservices.api.composite.product.ReviewSummary;
...
public class MessagingTests {
    ...
    void createCompositeProduct1() {
        ...
        Event<Integer,Product> expectedEvent = new Event(new Product(composite.getProductId(), composite.getName(), composite.getWeight(), null), composite.getProductId(), CREATE);
    ...
    void createCompositeProduct2() {
        ...
        Event<Integer, Product> expectedRecommendationEvent =
                new Event(
                        new Recommendation(composite.getProductId(), rec.getRecommendationId(), rec.getAuthor(), rec.getRate(), rec.getContent(), null),composite.getProductId(),CREATE);
        ...
        Event<Integer, Product> expectedReviewEvent =
                new Event(new Review(composite.getProductId(), rev.getReviewId(), rev.getAuthor(), rev.getSubject(), rev.getContent(), null),composite.getProductId(), CREATE);
    ...
    void deleteCompositeProduct() {
        ...
        Event<Integer, Product> expectedProductEvent = new Event(null, 1, DELETE);
        assertThat(productMessages.get(0), is(sameEventExceptCreatedAt(expectedProductEvent)));

        // Assert one delete recommendation event queued up
        assertEquals(1, recommendationMessages.size());

        Event<Integer, Product> expectedRecommendationEvent = new Event(null, 1, DELETE);
        assertThat(recommendationMessages.get(0), is(sameEventExceptCreatedAt(expectedRecommendationEvent)));

        // Assert one delete review event queued up
        assertEquals(1, reviewMessages.size());

        Event<Integer, Product> expectedReviewEvent = new Event(null, 1, DELETE);
        assertThat(reviewMessages.get(0), is(sameEventExceptCreatedAt(expectedReviewEvent)));
    }
    ...
    private Message<byte[]> getMessage(String bindingName){
        try {
            ...
            // If the messageQueues number variable in the target object contains no queues,
    ...
    private void deleteAndVerifyProduct(int productId, HttpStatus expectedStatus) {
        client.delete()
                .uri("/product-composite/{productId}", productId)
                .exchange()
                .expectStatus().isEqualTo(expectedStatus);
    }
