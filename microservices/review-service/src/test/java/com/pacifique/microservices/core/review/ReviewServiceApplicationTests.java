...
class ReviewServiceApplicationTests {
	...
	@Test
	void getReviewInvalidParameterNegative() {
		int productIdNegative = -1;

		client.get()
				.uri("/review?productId="+ productIdNegative)
				.exchange()
				.expectStatus().isEqualTo(UNPROCESSABLE_ENTITY)
				.expectHeader().contentType(APPLICATION_JSON)
				.expectBody()
				.jsonPath("$.path").isEqualTo("/review")
				.jsonPath("$.message").isEqualTo("Invalid productId: "+productIdNegative);
	}
